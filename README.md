...
*   **精确排列 (Precise Arrange)** ：勾选后，底部将启用“水平/垂直排列”按钮，支持按自定义“间距”数值进行精确排列。
*   **忽略链接 (Ignore Links)** ：操作时自动断开关联图层的链接，防止联动移动。
*   **忽略调整层 (Ignore Adjustments)** ：将调整图层（如曲线、色阶）排除在对齐操作之外。
*   **按组对齐 (Align by Group)** ：将组视为单个整体进行对齐，而不是展开组内的所有子图层。
